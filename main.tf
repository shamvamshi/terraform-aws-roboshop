resource "aws_lb_target_group" "main" {
  ...
  port     = local.tg_port # catalogue default port no is 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
...
resource "terraform_data" "main" {  # configuring data
  triggers_replace = [
...
resource "aws_ec2_instance_state" "main" { # stopping the catalogue instance
  instance_id = aws_instance.main.id
  state       = "stopped"
  depends_on = [terraform_data.main]
}

resource "aws_ami_from_instance" "main" { # taking ami from instance
  name               = "${var.project}-${var.environment}-${var.component}"
...
resource "terraform_data" "main_delete" {  # configuring data
  triggers_replace = [
    aws_instance.main.id
  ]
